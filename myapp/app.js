...
app.use(function(req, res, next) {
    res.status(404).render('error', { message: "La page que vous avez demandée n'existe pas." });
});
...
app.use(function(err, req, res, next) {
    ...
    // Réponse de l'erreur
    res.status(err.status || 500).render('error', { message: "Une erreur est survenue sur le serveur.", error: req.app.get('env') === 'development' ? err : {} });
});
